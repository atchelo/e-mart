...
class Subcategory extends Model
{
    ...
    public function childcategory(){
    	return $this->hasMany('App\Grandcategory','subcat_id','id');
    }
    ...
    public function getURL()
    {
        ...
        if (isset($item)) {
            ...
            $url = url('shop?category='.$item->category->id.'&sid='.$item->id.'&start='.sprintf('%.2f',$startp*$conversion_rate).'&end='.sprintf('%.2f',$endp*$conversion_rate));
