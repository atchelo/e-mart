...
<body>
	...
	<div class="container">
		<br>
		<a href="{{ route('seller.view.order',$order->order_id) }}" title="Go back"
		...
		<table class="table table-striped table-bordered">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						@if($orivar->variantimages)
							<img class="pro_img" width="70px" height="70px"
							src="{{url('variantimages/'.$orivar->variantimages['main_image'])}}"/>
						@else
							<img class="pro_img" width="70px" height="70px"
							src="{{ Avatar::create($orivar->products->name) }}"/>
						@endif
					</td>
				...
				<tr>
					...
					<td><b><i class="{{ $invoice->order->paid_in }}"></i>{{ $invoice->order->gift_charge }}</b></td>
