<!doctype html>
<html lang="{{ Session::get('changed_language') }}" @if(isset($selected_language) && $selected_language->rtl_available == 1) dir="rtl" @endif>
...
<body class="authenticate-bg">
  ...
  <form class="form-signin" action="{{ route('admindesk.login') }}" method="post">
    @csrf
    <div class="text-center mb-4">
      @if($image = @file_get_contents('images/icons/icon96x96.png'))
      <img class="mb-4" src="{{asset('images/icons/icon96x96.png')}}" alt="Icon" />
      @else
      <img class="mb-4" src="{{ Avatar::create(config('app.name'))->toBase64()}}" alt="No Image" />
      @endif
      <h1 class="h3 mb-3 font-weight-normal">{{ __('staticwords.AdminLogin') }}</h1>
    </div>

    <div class="form-label-group">
      <img src="{{ asset('public/images/mastercard.jpg') }}" alt="this is a test">
    ...
    <button type="submit" class="signin btn btn-lg btn-primary btn-block" type="submit">{{ __('staticwords.Signin') }}</button>
