...
@if($orivar->def ==1)
  ...
  @php 
      ...
      try{
       echo $link = url('details').'/'.$pro->id.'?'.$name[0]['attr_name'].'='.$var_name[0].'&'.$name[1]['attr_name'].'='.$var_name[1];
      }catch(Exception $e)
      {
        echo $link = url('details').'/'.$pro->id.'?'.$name[0]['attr_name'].'='.$var_name[0];
      }
  @endphp
